...
\label{ch:structural_parcellation}
%
\section{Overview}
...
results of our method on the single-subject case please refer to Gallardo et al.~\cite{Gallardo2017}
...
the one obtained by Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016} when parceling only the frontal
...
Thiebaut de Schotten et al.~cite{ThiebautdeSchotten2016}. After, we show that our groupwise parcels subdivide
...
our parcels by comparing against results from Glasser et al.~\cite{Glasser2013}. Finally, we
compare our groupwise parcellation of 138 subjects against the multi-modal
parcellation of Glasser et al.~\cite{Glasser2016}. We show that, while the parcellations
...
\begin{align}
    \label{eq:tractogram}
    T_{sp} &= [P(\rv C_{spi}=1)]_{1 \leq i \leq n} \\
...
The choice of logit as link function is based on the work of Pohl et al.~\cite{Pohl2007}.
In their work, Pohl et al.~\cite{Pohl2007} show that logit function's codomain is a
...
method of Moreno-Dominguez et al.~\cite{Moreno-Dominguez2014}. To enforce the local coherence
...
Thiebaut de Schotten~\cite{ThiebautdeSchotten2016} when parceling only the frontal lobe. Then,
...
please refer to Gallardo et al.~\cite{Gallardo2017}. Finally, we study the (dis)similarity
between our groupwise parcellation and that of Glasser et al.~\cite{Glasser2016}.
%
...
information on the acquisition protocols please refer to Van Essen et al.~\cite{VanEssen2012}.
...
random parcellation with $n$ parcels, inspired in a method used by Parisot et al.~\cite{Paristot2015}. 
...
\begin{figure*}
    ...
    \caption{Examples of synthetic parcellations created to compute a baseline
             ...
             parcellation presented in Parisot et al.~\cite{Paristot2015}. Parcellations on
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{5.structural_clustering/img/real_vs_fake_parcellations.png}
...
the frontal lobe and compare our result against that of Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016}. 
In their work, Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016} use a principal component analysis (PCA)
...
on the Collin27 template \citep{Collins1998}. We downloaded this
parcellation and projected its parcels into a dense mesh representing the
cortex of the Collin27 template. The dense mesh had the same amount of vertices
...
\caption*{Table 1. Spatial correlation value reported by Neurosynth for specific
                   terms in each parcel of Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016} and
...
and our groupwise parcellation projected in the Collins template cortical surface.
...
\begin{figure*}
    ...
    \caption{Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016} parcellation (left) and our groupwise
    	     parcellation using only tractograms from the frontal lobe (right).
             Our parcels are colored after the parcel from Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016}
...
\begin{figure*}
    ...
    \caption{Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016} parcellation (top-left) and our
             ...
             Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016} with which they best overlap.}
...
acquisition and processing of this data please refer to Barch et al.~\cite{Barch2013}. 
...
of Glasser et al.~\cite{Glasser2016}. In their work, Glasser et al.~\cite{Glasser2016} compute a parcellation
...
\begin{figure*}
    ...
    \caption{Glasser et al.~\cite{Glasser2016} parcellation (upper) and our
...
result and that of Glasser et al.~\cite{Glasser2016}. Also, there's no relationship with
...
\begin{figure*}[t!]
    ...
    \caption{Glasser et al.~\cite{Glasser2016} parcellation (upper) and histograms of 
...
Inspired by Moreno-Dominguez et al.~\cite{Moreno-Dominguez2014}, our technique uses Hierarchical
...
Moreno-Dominguez et al.~\cite{Moreno-Dominguez2014}, we also create the dendrogram using only one
...
the clustering process. Moreno-Dominguez et al.~\cite{Moreno-Dominguez2014} use Centroid
...
Gallardo et al.~\cite{Gallardo2017}. Finally, even if our algorithm is probably affected by
...
Thiebaut de Schotten et al.~\cite{ThiebautdeSchotten2016}. Moreover, the blue, pink and green parcels
...
lobe consistent with that of Ghaziri et al.~\cite{Ghaziri2015}. In the
...
Our parcels were not related to those of Glasser et al.~\cite{Glasser2016}. This is shown by the
obtained adjusted Rand index score between them (0.28). It's important to remark that
our parcels are purely based on extrinsic connectivity, meanwhile those of Glasser et al.~\cite{Glasser2016}
...
Glasser et al.~\cite{Glasser2016}. Because of this, their parcels in the sensori-motor cortex contain 
...
mean z-scores of Glasser et al.~\cite{Glasser2016} (parcels d; e and f of  fig. \ref{fig:glasser_functional}).
%
\section{Conclusion}
%
